...
class MusicPlayer:
    ...
    def load_config(self):
        # Load the music directory from configuration file.
        if os.path.isfile(CONFIG_FILE):
            ...
            if os.path.isdir(path):
                self.music_dir = path
                return
        # If config file does not exist or path is invalid, ask for input.
    ...
    def random_track(self):
        # Randomly select a track with an index different from the current one and play it
    ...
    def monitor_playback(self):
        """在后台线程中检测音乐是否播放完毕，若播放完自动播放下一首"""
        while True:
            # 当音乐未暂停且当前没有播放（已结束），则自动切换到下一首。
